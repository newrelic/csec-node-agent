...
function wrapStart(shim, original) {
  ...
  return function wrappedStart() {
    ...
    // const originalListener = arguments[1]
    // const csecListener = Object.assign({}, originalListener);

    // csecListener.onReceiveStatus = (status) => {
    //   if (originalListener && originalListener.onReceiveStatus) {
    //     const onReceiveStatuts = originalListener.onReceiveStatus;
    //     onReceiveStatuts(status)
    //   }
    // }

    // csecListener.onReceiveMessage = (data) => {
    //   logger.debug("data is:", data.toString());
    //   if (originalListener && originalListener.onReceiveMessage) {
    //     const onReceiveMessages = originalListener.onReceiveMessage;
    //     onReceiveMessages(data)
    //   }
    // }

    // csecListener.onReceiveMetadata = (metadata) => {
    //   logger.debug("metadata is:", metadata);
    //   if (originalListener && originalListener.onReceiveMetadata) {
    //     const onReceiveMetadataa = originalListener.onReceiveMetadata;
    //     onReceiveMetadataa(metadata)
    //   }
    // }
    // arguments[1] = csecListener
...
function wrapHandler(shim, mod, method) {
  shim.wrap(mod, method, function wrapHandler(shim, fn) {
    ...
    return function wrappedHandler() {
      ...
      logger.debug("args:", arguments[0]);
      wrapFoo(shim, arguments[0]);
...
function wrapFoo(shim, stream) {
  shim.wrap(stream, 'on', function wrapStreamEvent(shim, fn) {
    if (!shim.isFunction(fn)) {
      return fn
    }
    return function wrappedStreamEvent() {
      let incomingData;
      ...
      if (segment && segment.transaction) {
        ...
        const type = stream.call.handler.type;
        const path = stream.call.handler.path;
        reqData.method = path;
        reqData.url = path;
        reqData.uri = path;
        reqData.headers = headers
        if (reqData.headers && reqData.headers[NR_CSEC_FUZZ_REQUEST_ID]) {
          let bufferObj = Buffer.from(data.headers[NR_CSEC_FUZZ_REQUEST_ID], "base64");
          let decodedString = bufferObj.toString("utf8");
          reqData.headers[NR_CSEC_FUZZ_REQUEST_ID] = decodedString;
        }
        reqData.type = type;
        reqData.protocol = 'grpc';
        reqData.isGrpc = true;
        reqData.body = null;
        reqData.serverPort = shim.serverPort;
        transactionId = segment.transaction.id;
      }
      if (arguments[0] == 'data') {
        shim.wrap(arguments, '1', function wrapData(shim, fn) {
          if (!shim.isFunction(fn)) {
            return fn
          }
          return function wrappedData() {
            incomingData = arguments[0];
            if(transactionId && incomingData){
              reqData.body = JSON.stringify(incomingData)
              requestManager.setRequest(transactionId, reqData);
            }
            console.log("data is:",transactionId, reqData);
