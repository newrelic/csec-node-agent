...
module.exports.wrapServer = function wrappedServer(shim, server) {
  ...
  logger.info("Instrumenting @grpc/grpc-js")
    ...
    for (const [key, value] of Object.entries(result)) {
      if (!value.type && value.serviceName) {
        ...
        for (const [innerKey, innerValue] of Object.entries(value)) {
          if (!innerValue.type && innerValue.serviceName) {
            grpcutils.setService(innerValue.serviceName, serviceObject);
            serviceUtil(innerValue);
          }
        }
      }
