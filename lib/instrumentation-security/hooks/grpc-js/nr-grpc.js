...
function addRequestData(shim, type, path, args) {
  ...
  try {
    ...
    if (segment && segment.transaction) {
      ...
      if (data.headers && data.headers['nr-csec-fuzz-request-id']) {
        let bufferObj = Buffer.from(data.headers['nr-csec-fuzz-request-id'], "base64");
        let decodedString = bufferObj.toString("utf8");
        data.headers['nr-csec-fuzz-request-id'] = decodedString;
      }
      data.type = type;
      data.protocol = 'grpc';
      data.isGRPC = true;
