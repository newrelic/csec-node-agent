  ...
  shim.wrap(mod, methodName, function makeWrapper(shim, fn) {
    return function wrapper() {
      ...
      try {
        ...
        let completeURL = data.protocol + DOUBLE_SLASH + data.headers.host + (data.port ? (COLON + data.port) : EMPTY_STRING);
        completeURL = completeURL.split(QUESTION_MARK)[0];
        shim.interceptedArgs = data;

        if (request && !lodash.isEmpty(data.headers.host)) {
        ...
        logger.debug("Error while extracting undici args:", error);
      }
