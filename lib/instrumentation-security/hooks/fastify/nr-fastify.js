    ...
    shim.wrap(fastify, fun, function wrapRoute(shim, fn) {
      ...
      return function wrappedRoute() {
        try {
          ...
          const key = splittedStack[1].toUpperCase() + ATTHERATE + arguments[0];
...
function extractParams(shim, req) {
  try {
    ...
    if (transaction) {
      ...
      const symbolKey = Object.getOwnPropertySymbols(req).find(symbol => symbol.toString() === 'Symbol(fastify.context)');
      const txInfo = symbolKey ? req[symbolKey] : undefined;
