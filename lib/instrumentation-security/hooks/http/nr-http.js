...
function outboundHook(shim, mod, method, moduleName) {
  if (API && API.getNRAgent() && API.getNRAgent().config.security.skip_iast_scan.iast_detection_category.ssrf) {
  ...
  shim.wrap(mod && mod.Server && mod.Server.prototype, 'emit', function makeEmitWrapper(shim, fn) {
    ...
    return function emitWrapper() {
      ...
      if (arguments[0] == 'request') {
        if (NRAgent && NRAgent.config.security.detection.rxss.enabled && !NRAgent.config.security.skip_iast_scan.iast_detection_category.rxss) {
