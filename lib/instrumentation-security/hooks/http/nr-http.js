...
const { NR_CSEC_FUZZ_REQUEST_ID, QUESTION_MARK, EMPTY_STRING, UTF8, CONTENT_TYPE, TEXT_HTML, APPLICATION_JSON, APPLICATION_XML, APPLICATION_XHTML, TEXT_PLAIN, APPLICATION_X_FORM_URLENCODED, MULTIPART_FORM_DATA, STRING, COMMA_SPACE, SEMI_COLON, EQUAL, SECURE } = require('../../core/constants');
...
function responseHook(resp, req, shim) {
  ...
  shim.wrap(resp, 'setHeader', function makeSetHeaderWrapper(shim, fn) {
    return function wrappedSetHeader() {
      if (arguments[0] && arguments[0] === SET_COOKIE) {
        const request = requestManager.getRequest(shim);
        if (request) {
          secureCookieCheck(arguments[1], request, shim)
        }
      }
      return fn.apply(this, arguments);
    }
...
function secureCookieCheck(cookieString, csec_request, shim) {
  if (cookieString) {
    try {
      if (typeof cookieString !== STRING) {
        cookieString = cookieString.join(COMMA_SPACE);
      }
      ...
      let parameter = secureFlag ? "true" : "false"
      const args = [parameter];
      const traceObject = secUtils.getTraceObject(shim);
      const secMetadata = securityMetaData.getSecurityMetaData(csec_request, args, traceObject, secUtils.getExecutionId(), EVENT_TYPE.SECURE_COOKIE, EVENT_CATEGORY.SECURE_COOKIE)
      const secEvent = API.generateSecEvent(secMetadata);
      API.sendEvent(secEvent);
    } catch (error) {
    }
