...
const { NR_CSEC_FUZZ_REQUEST_ID, QUESTION_MARK, EMPTY_STRING, UTF8, CONTENT_TYPE, TEXT_HTML, APPLICATION_JSON, APPLICATION_XML, APPLICATION_XHTML, TEXT_PLAIN, APPLICATION_X_FORM_URLENCODED, MULTIPART_FORM_DATA, COMMA, SEMI_COLON, EQUAL, SECURE, HTTPONLY, SAMESITE, STRICT } = require('../../core/constants');
...
const maxCollectionLimit = 60;
const minimumCollectionLimit = 10;
const samplesPerInterval = 2;
let sampleIndex = 0;
let collectedRequestPerSecond = 0;
const maxCollectedRequestPerSecond = 5;

if (NRAgent) {
  setInterval(() => {
    logger.debug("collectedRequests:", collectedRequests)
    collectedRequests = 0;
    sampleIndex = 0;
    shouldProcess = true;
  }, 60000);
  ...
  setInterval(() => {
    logger.debug("collectedRequestPerSecond:", collectedRequestPerSecond);
    collectedRequestPerSecond = 0;
  }, maxCollectedRequestPerSecond * 1000);
}
...
//   if (collectedRequests >= maxCollectionLimit) {
//     logger.debug("max request collection limit reached", collectedRequests);
//     shouldProcess = false;
//   }
//   if (collectedRequests > sampleIndex) {
//     shouldProcess = false;
//   }
//   else {
...
//   if (collectedRequests < minimumCollectionLimit) {
//     shouldProcess = true;
//   }

//   logger.debug("shouldProcess:", shouldProcess, collectedRequests)
// }, 2000);
...
function outboundHook(shim, mod, method, moduleName) {
  ...
  if (API && API.getNRAgent() && API.getNRAgent().config.security.mode == 'IAST_MONITORING') {
    logger.warn('ssrf detection is disabled');
    return;
  }
      ...
      if (arguments[0] == 'request') {
        ...
        shim.wrap(req, 'on', function makeOnWrapper(shim, fn) {
          ...
          return function OnWrapper() {
            if (shouldProcess && collectedRequests < maxCollectionLimit && collectedRequestPerSecond < maxCollectedRequestPerSecond) {
              collectedRequests++;
              collectedRequestPerSecond++;
              ...
              logger.debug("Discarding request from processing", collectedRequests);
            }
    ...
    return function wrappedCreateServer() {
      shim.wrap(arguments, '0', function appWrapper(shim, fn) {
        ...
        return function wrappedApp() {
          let req = arguments[0];
          if (!requestManager.getRequest(shim) && shouldProcess && collectedRequests < maxCollectionLimit && collectedRequestPerSecond < maxCollectedRequestPerSecond) {
            addRequestData(shim, req);
          }
          return fn.apply(this, arguments);
        }
 ...
 * @returns 
 */
function isUnsupportedContentType(conType) {
  try {
    if (!conType) {
      return true;
    }
