...
const ServiceMap = require('../../core/serviceMap');
...
function outboundHook(shim, mod, method, moduleName) {
  ...
  shim.wrap(mod, method, function makeRequestWrapper(shim, fn) {
    ...
    return function requestWrapper() {
      ...
      if (typeof data.headers.host == 'object') {
        data.headers.host = data.headers.host.host ? data.headers.host.host : EMPTY_STRING;
      }
      ...
      const connectionObject = {
        ...
        connectionURL: completeURL,
        type: 'HTTP',
        direction: "OUTBOUND"
      };

      ServiceMap.addOutboundConnection(connectionObject);
      ...
      if (request && !lodash.isEmpty(data.headers.host) && !lodash.isEmpty(data.path)) {
        ...
        if (arguments[0] instanceof Object) {
          if (arguments[0] && arguments[0].headers && request.headers) {
            ...
            arguments[0].headers[NRCSECTRACINGDATA] = traceHeader + appUUID + '/' + this.secEvent.apiId + '/' + this.secEvent.id + '/' + this.secEvent.linkingMetadata['entity.guid'] + ';';
          } else {
            arguments[0].headers = {};
            arguments[0].headers[NRCSECTRACINGDATA] = traceHeader + appUUID + '/' + this.secEvent.apiId + '/' + this.secEvent.id + '/' + this.secEvent.linkingMetadata['entity.guid'] + ';';
            arguments[0].headers.SECHOOK = id;
...
function addIncomingConnection(data) {
  const inBoundConnection = {};
  inBoundConnection.url = data.url.split(QUESTION_MARK)[0];
  inBoundConnection.sourceIp = '*';
  inBoundConnection.sourcePort = '';
  inBoundConnection.destinationIp = "localhost";
  inBoundConnection.destinationPort = data.serverPort;
  inBoundConnection.direction = "INBOUND";
  if (data.headers['nr-csec-tracing-data']) {
    let tracingHeaderList = data.headers['nr-csec-tracing-data'].split(";");
    // const sourceTracingHeader = tracingHeaderList[tracingHeaderList.length-1];
    tracingHeaderList.pop();
    const sourceTracingHeader = tracingHeaderList[tracingHeaderList.length - 1];
    const sourceTracingHeaderSplitted = sourceTracingHeader.split('/');

    const sourceUUID = sourceTracingHeaderSplitted[0];
    const sourceEntityGuid = sourceTracingHeaderSplitted[sourceTracingHeaderSplitted.length-1];
    inBoundConnection.sourceUUID = sourceUUID;
    inBoundConnection.sourceEntityGuid  = sourceEntityGuid;
  }
  ServiceMap.addInboundConnection(inBoundConnection);
