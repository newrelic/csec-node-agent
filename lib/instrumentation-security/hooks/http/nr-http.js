...
const errorUtils = require('../../../nr-security-agent/lib/core/errorUtils');
...
function processErrors(transaction, requestData) {
  if (transaction && transaction.userErrors.length > 0) {
    console.log("here i am:", transaction.userErrors[0]);
  }
  if (transaction && transaction.exceptions.length > 0) {
    for (i = 0; i < transaction.exceptions.length; i++) {
      console.log("exception is:", transaction.exceptions[0])
      errorUtils.generateErrorEvent(transaction.exceptions[0].error, requestData)
    }
  }
}
...
function setupRequestMapCleanup() {
  if (NRAgent) {
    NRAgent.on('transactionFinished', function onTransactionFinished(transaction) {
      ...
      processErrors(transaction, requestData)
