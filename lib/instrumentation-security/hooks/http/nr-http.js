...
function responseHook(resp, req, shim) {
  ...
  shim.wrap(resp, 'end', function makeEndWrapper(shim, fn) {
    ...
    return function wrappedEnd() {
      ...
      if (request && API.getSecAgent() && NRAgent.config.security.scan_controllers && NRAgent.config.security.scan_controllers.report_http_response_body) {
        const fuzzHeader = request.headers[NR_CSEC_FUZZ_REQUEST_ID];
        if (fuzzHeader.includes('VULNERABLE'))
...
function generateTransactionEvent(request, response) {
  ...
  transactionEvent.httpResponse.body = response.res.body;
