  ...
  shim.wrap(resp, 'end', function makeEndWrapper(shim, fn) {
    ...
    return function wrappedEnd() {
      ...
      try {
        ...
        if (parseInt(responseCode / 100) == 5 && request) {
...
/**
 * Collect and report  the uncuaght Exception
 */
process.on('uncaughtException', (err) => {
  ...
  if (exception) {
    ...
    logger.debug("Reporing error:", JSON.stringify(exception));
  }
  //Need to crash the app once uncaught exception is raised
  setTimeout(() => {
    console.error(err);
    process.exit(1);
  }, 1000);
