...
function setupRequestMapCleanup() {
  if (NRAgent) {
    NRAgent.on('transactionFinished', function onTransactionFinished(transaction) {
      logger.debug("transaction:", transaction.exceptions, transaction.userErrors, NRAgent.config.error_collector.enabled);
  ...
  shim.wrap(resp, 'end', function makeEndWrapper(shim, fn) {
    ...
    return function wrappedEnd() {
      ...
      if(txInfo && txInfo.error && request){
        logger.debug("error found:", txInfo.error, txInfo.error.stack)
        const exception = new ExceptionReporting.exceptionReporting(txInfo.error, request);
