...
function responseHook(resp, req, shim) {
  ...
  shim.wrap(resp, 'end', function makeEndWrapper(shim, fn) {
    return function wrappedEnd() {
      ...
      responseBodyCompute(response, arguments);
      const request = requestManager.getRequest(shim);
      secureCookieCheck(response, request)
...
function secureCookieCheck(resp, csec_request) {
  ...
  if (cookieString) {
    try {
      ...
      console.log(cookieObject, secureFlag);
      if(csec_request.protocol===HTTP_PROTO && !secureFlag){
        logger.debug("Not a secure cookie", cookieObject);
      }
    } catch (error) {
      console.log(error);
    }
