...
function initialize(shim) {
    ...
    Module._load = function (request, parent) {
        ...
        let flag = false;
        if (moduleName && typeof (moduleName) === STRING && !moduleName.includes(CSEC)) {
            flag = true;
        }
        else if (parent && parent.id && typeof (parent.id) === STRING && moduleName.includes(CSEC) && parent.id.includes(CSEC)) {
            flag = false;
        }
        else if (parent && parent.id && moduleName && typeof (moduleName) === STRING && moduleName.includes(CSEC) && parent.id.includes(CSEC)) {
            flag = false;
        }
        if (flag) {
