...
module.exports = function initialize(shim, AWS, moduleName) {
  ...
  const docClientProto = dynamoDB.DocumentClient.prototype
  makeRequestHook(shim, dynamoDB && dynamoDB.prototype, 'makeRequest');

  DOC_CLIENT_OPERATIONS.forEach(function (fun) {
    docClientHooks(shim, docClientProto, fun);
  })

  shim.wrapReturn(AWS, 'DynamoDB', function wrapDynamo(shim, fn, name, ddb) {
    DDB_OPERATIONS.forEach(function (fun) {
      docClientHooks(shim, ddb, fun);
    })
  })
