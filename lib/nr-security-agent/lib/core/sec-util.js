...
const restictionUtils = require('./restrictionUtils');
...
function mergeObjects(obj1, obj2) {
    ...
    try {
        ...
        // Iterate over keys and merge arrays
        keys.forEach((key) => {
...
function addSkipScanParameters(secEvent) {
    try {
        const skipScanParameters = {
            header: [],
            query: [],
            body: [],
            path: []
        };
        const mappingParameters = NRAgent.config.security.restriction_criteria.mapping_parameters
        if (mappingParameters.header.enabled) {
            skipScanParameters.header = mappingParameters.header.location;
        }
        if (mappingParameters.query.enabled) {
            skipScanParameters.query = mappingParameters.query.location;
        }
        if (mappingParameters.body.enabled) {
            skipScanParameters.body = mappingParameters.body.location;
        }
        if (mappingParameters.path.enabled) {
            skipScanParameters.path = NRAgent.config.security.restriction_criteria.account_info.account_id_values;

        }
        secEvent.metaData.skipScanParameters = mergeObjects(skipScanParameters, secEvent.metaData.skipScanParameters)
    } catch (error) {
        logger.debug('Error in adding skip scan parameters', error);
    }
...
function sendEvent(secEvent) {
    ...
    let restrictToAccountFlag = false;
    let canGenerateEvent = false;
    if (commonUtils.getCSECmode() === 'IAST_RESTRICTED') {
        restrictToAccountFlag = true;
        canGenerateEvent = restictionUtils.isHttpRequestRestictToAccount(secEvent.httpRequest);
        addSkipScanParameters(secEvent);
    }

    if (restrictToAccountFlag && !canGenerateEvent) {
        return;
    }
