...
function sendEvent(secEvent) {
    let sendEventFlag = true;
    if (secEvent && secEvent.apiId) {
        sendEventFlag = checkToSendEvent(secEvent.apiId)
    }

    const agentModule = Agent.getAgent();
    if (agentModule.status.getStatus() != 'connecting' && NRAgent.config.security.enabled && agentModule.status.getStatus() == 'active') {
        if (commonUtils.getCSECmode() == 'IAST_MONITORING' && sendEventFlag) {
            require('./connections/restclient').sendSecurityEvent(secEvent);
        }
        else {
...
/**
 * Utility to check whether to send event or not based on event quota per trace
 * @param {*} apiId 
 * @returns 
 */
function checkToSendEvent(apiId) {
    try {
        if (eventQuotaMap.has(apiId)) {
            eventQuotaMap.set(apiId, eventQuotaMap.get(apiId) + 1);
        } else {
            eventQuotaMap.set(apiId, 1);
        }
        if (eventQuotaMap.get(apiId) < NRAgent.config.security.monitoring_criteria.event_quota_per_trace) {
            return true;
        } else {
            return false;
        }
        ...
        logger.debug("Error while checking the event quota per trace", error);
        return false;
    }
}

/**
 * utility to clear event quota map
 */
function clearEventQuota() {
    let event_quota_time_duration = NRAgent.config.security.monitoring_criteria.event_quota_time_duration * 60 * 1000;
    let repeat = NRAgent.config.security.monitoring_criteria.repeat;
    setInterval(() => {
        eventQuotaMap.clear();
        repeatCount++;
    }, event_quota_time_duration);

    if (repeatCount == repeat && repeat != 0) {
        commonUtils.shutDownAgent();
    }
