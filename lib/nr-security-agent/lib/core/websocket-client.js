...
const sendHC = async function () {
    if (AgentStatus.getInstance().getStatus() != AgentStatus.CSECAgentStatus.codes.DISABLED && (NRAgent ? (NRAgent.config.security.enabled && NRAgent.canCollectData()) : true)) {
        ...
        json.exitEventSentCount = json.eventStats.exitEventStats.sent;
        json.eventRejectionCount = json.eventStats.raspEventStats.rejected + json.eventStats.iastEventStats.rejected;
        json.eventProcessingErrorCount = json.eventStats.raspEventStats.errorCount + json.eventStats.iastEventStats.errorCount;
