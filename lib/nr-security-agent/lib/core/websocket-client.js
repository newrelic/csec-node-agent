...
const sendHC = async function () {
    if (AgentStatus.getInstance().getStatus() != AgentStatus.CSECAgentStatus.codes.DISABLED && (NRAgent ? (NRAgent.config.security.enabled && NRAgent.canCollectData()) : true)) {
        ...
        if (NRAgent && NRAgent.config) {
            hc.getInstance().appEntityGuid = NRAgent.config.entity_guid;
            hc.getInstance().appAccountId = NRAgent.config.account_id;
        }
        ...
        hc.getInstance().scanStartTime =  commonUtils.scanStartTime;
        hc.getInstance().trafficStartedTime =  commonUtils.trafficStartedTime;
        hc.getInstance().iastTestIdentifer = getTestIdentifier();
