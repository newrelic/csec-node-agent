...
function getSHA256ForData (data) {
    try {
        if (!process.traceMap) {
            return hash.sha256().update(data).digest(HEX_DIGEST); ;
        }
        const cachedSha = process.traceMap.get(data);

        if (cachedSha) {
            return cachedSha;
        } else {
            const calSha = hash.sha256().update(data).digest(HEX_DIGEST);
            if (process.traceMap.size < 2000 && (httpreq && httpreq.data)) {
                process.traceMap.set(data, calSha);
            }
            return calSha;
        }
    } catch (err) {
        logger.error('Unable to calculate SHA256 for data.');
        logger.debug(err);
        return null;
    }
}
