...
setTimeout(() => {
    setInterval(() => {
       ...
       let timeDiffInSeconds = (currentTiime - lastFuzzEventTime)/1000
       logger.debug("timeDiffInSeconds:",timeDiffInSeconds);
...
function handleFuzzResponse (response, fuzzDetails) {
    ...
    if (response) {
        ...
        logger.debug("fuzzRequest id:",IASTUtil.generateIASTDataRequest());
        response.then(() => {
