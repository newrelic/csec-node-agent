...
function startIASTSchedular() {
    ...
    iastIntervalConst = setInterval(() => {
        ...
        logger.debug("Time difference since last fuzz request:", timeDiffInSeconds);
        logger.debug("Completed requests so far:", completedListSize);
        logger.trace("Pending requests so far:", pendingListSize);
        logger.info("additionalCoolDownTime:", additionalCoolDownTime);

        if (timeDiffInSeconds > 1 && additionalCoolDownTime == 0) {
            Agent.getAgent().client.dispatcher(data);
            logger.info("Sending iast-data-request");
        }
