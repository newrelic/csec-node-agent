...
function handler(json) {
    HC.getInstance().iastReplayRequest.receivedControlCommands++;
...
function handleFuzzRequest(fuzzDetails) {
    ...
    HC.getInstance().iastReplayRequest.processedControlCommands++;
    try {
        if (fuzzRequest.protocol == 'grpc') {
            ...
            HC.getInstance().iastReplayRequest.replayRequestGenerated++;
            grpcClient.fireRequest(config);
            HC.getInstance().iastReplayRequest.replayRequestExecuted++;
...
function handleFuzzResponse(response, fuzzDetails) {
    ...
    if (response) {
        ...
        response.then(() => {
            ...
            HC.getInstance().iastReplayRequest.replayRequestSucceeded++;
        }).catch(() => {
            logger.debug('Error occured:', fuzzRequest.url, fuzzRequest);
            HC.getInstance().iastReplayRequest.replayRequestFailed++;
        });
    }
}
