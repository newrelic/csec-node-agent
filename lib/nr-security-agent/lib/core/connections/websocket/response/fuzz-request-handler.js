...
function getFuzzIdentifier(fuzzHeader) {
    try {
        let apiId = fuzzHeader.split(CSEC_SEP)[0]
        if (apiId && !fuzzedApiIDSet.has(apiId) ) {
            fuzzedApiIDSet.add(apiId);
            logger.info("IAST Scan for API ID : %s started.", apiId);
            logger.info("fuzzedApiIDSet size", fuzzedApiIDSet.size);
        }
    } catch (error) {
    ...
    try {
        if (IS_LAMBDA_ENV) {
            ...
            if(fuzzRequest.headers && fuzzRequest.headers[NR_CSEC_FUZZ_REQUEST_ID]){
                getFuzzIdentifier(fuzzRequest.headers[NR_CSEC_FUZZ_REQUEST_ID])
            }
