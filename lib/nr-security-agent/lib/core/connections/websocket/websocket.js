...
SecWebSocket.prototype.init = function init() {
    ...
    initLogger.info(`[STEP-4][BEGIN][WS] Connecting to Prevent-Web service at ${validatorService}`);
    let cert = ''
    try {
...
const defaultOnOpenCB = () => async function onOpen() {
    ...
    try {
        initLogger.info(LOG_MESSAGES.WS_CONNECTED);

        this.wsRetryCount = 0;
        process.env.WS_HEALTH = 'OK';

        const applicationInfo = APP_INFO || Agent.getAgent().applicationInfo;
        initLogger.info(LOG_MESSAGES.SENDING_APPINFO, JSON.stringify(applicationInfo));
