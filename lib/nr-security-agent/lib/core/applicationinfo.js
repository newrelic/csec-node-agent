...
function getSubnet() {
    ...
    Object.keys(interfaces).forEach((data) => {
        ...
        interfaces[data].forEach((addr) => {
          if (addr.family === 'IPv4') {
            const subnetMask = addr.netmask;
            const ipAddr = addr.address;
            const subnetAddr = getSubnetAddress(ipAddr, subnetMask);
            logger.debug("interface:",addr);
            logger.debug(`Calculated Subnet address for ${ipAddr} is: ${subnetAddr}`);
          }
        });
      });
}
...
function getSubnetAddress(ipAddr, subnetMask) {
    const ipBytes = ipAddr.split('.').map((byte) => parseInt(byte, 10));
    const maskBytes = subnetMask.split('.').map((byte) => parseInt(byte, 10));
    for (let i = 0; i < 4; i++) {
