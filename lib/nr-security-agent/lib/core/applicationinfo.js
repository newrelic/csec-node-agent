...
ApplicationInfo.prototype.updateSHAAndSize = async function updateSHAAndSize (GlobalPromise) {
    ...
    if (Agent.getAgent()) {
        ...
        initLogger.info(LOG_MESSAGES.APP_INFO_GENERATED, JSON.stringify(applicationInfo));
    }
};
...
function setBuildDetails (agentDir, applicationInfo) {
    ...
    process.env.collectorVersion = pkg.collectorVersion;
    applicationInfo.collectorVersion = pkg.collectorVersion;
    applicationInfo.jsonVersion = pkg.jsonVersion;
    applicationInfo.buildNumber = pkg.buildNumber;
    process.env.buildNumber = pkg.buildNumber;
    process.env.jsonVersion = applicationInfo.jsonVersion;
}
...
function getHostName () {
    try {
        // const hostFile = fs.readFileSync("/etc/hostname");
        // return hostFile.toString().trim();
        return OS.hostname();
    } catch (err) {
        return EMPTY_STRING;
    }
