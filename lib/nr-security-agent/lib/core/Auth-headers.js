...
function AuthHeaders() {
    try {
        ...
        authHeaders['NR-CSEC-IAST-SCAN-INSTANCE-COUNT'] = NRAgent.config.security.scan_controllers.scan_instance_count ? NRAgent.config.security.scan_controllers.scan_instance_count : -1
        if ((NRAgent.config.security.iast_test_identifier && !lodash.isEmpty(NRAgent.config.security.iast_test_identifier)) || process.env.NEW_RELIC_SECURITY_IAST_TEST_IDENTIFIER) {
            ...
            logger.debug("iast_test_identifier:", NRAgent.config.security.iast_test_identifier, iast_test_identifier);
