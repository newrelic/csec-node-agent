...
function AuthHeaders() {
    try {
        ...
        authHeaders['NR-AGENT-RUN-TOKEN'] = process.env.agent_run_id || NRAgent.config.run_id;
        const COLLECTOR_VERSION = process.env.collectorVersion || appInfo.collectorVersion || '';
        ...
        authHeaders['NR-CSEC-JSON-VERSION'] = process.env.jsonVersion || appInfo.jsonVersion;
        authHeaders['NR-CSEC-BUILD-NUMBER'] = process.env.buildNumber || appInfo.buildNumber;
        authHeaders['NR-ACCOUNT-ID'] = process.env.account_id || NRAgent.config.account_id;
...
function getInstance() {
    if (!authHeaders) {
        ...
        initLogger.info("[STEP-2][BEGIN][Auth-Headers]: Authentication Headers Created Successfully");
    }
    return authHeaders;
}
