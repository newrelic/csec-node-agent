...
function getXSSConstructs (data) {
    ...
    while (currPos < data.length && typeof data == STRING ) {
        ...
        while ((attribMatcher = attribRegex.exec(data)) !== null) {
            ...
            let attribData = attribMatcher[0].trim();
            ...
            if ((tmpCurrPos === -1 || attribMatcher.index < tmpCurrPos)) {
                ...
                tmpStartPos++;
                if (!attribMatcher[3] && attribRegex.lastIndex >= tmpCurrPos) {
                ...
                if (key && (upperCaseAttribKey.startsWith('ON') || upperCaseAttribKey === 'SRC' || upperCaseAttribKey === 'HREF' ||
