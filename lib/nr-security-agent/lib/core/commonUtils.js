...
function getValidatorServiceEndpointURL() {
    ...
    if (!validatorServiceEndpointURL) {
...
function executeCronSchedule() {
    try {
        ...
        logger.info("Security Agent scheduled time is set to:", schedule);
        const { Agent } = require('./agent');
        Agent.getAgent().delayed = true
        cronJobInstance = nodeCron.schedule(
            schedule,
            () => {
                logger.info('Cron schedule invoked at the schedule:',schedule);
                refreshAgent();
             },
             {
                 scheduled: true,
        ...
        logger.debug("Cron job instance created successfully");
    } catch (error) {
