...
const { CSEC_HOME, SLASH, EMPTY_STR, APPLICATION_JSON, APPLICATION_XML } = require('./sec-agent-constants');
...
function executeCronSchedule() {
    const schedule = NRAgent.config.security.scan_schedule.schedule;
...
function honourDurationConfiguration() {
    const duration = NRAgent.config.security.scan_schedule.duration;
    if (duration < 1) {
        return;
    }
    const { Agent } = require('./agent');
...
function isValuePresentInAccountIds(value) {
    const accountIdValues = NRAgent.config.security.restriction_criteria.account_info.account_id_value;
...
function checkForAccountRestrictionToQuery(request) {
    ...
    try {
        const queryMappingParametersList = NRAgent.config.security.restriction_criteria.mapping_parameters.query;
...
function checkForAccountRestrictionToHeader(request) {
    ...
    try {
        const headerMappingParametersList = NRAgent.config.security.restriction_criteria.mapping_parameters.header;
...
function checkForAccountRestrictionToJsonBody(request) {
    let restictBodyFlag = false;
    try {
        const jsonBody = parseJsonString(request.body);
        if (!jsonBody) {
            return false;
        }
        let keyValuePairs = getAllKeyValuePairs(jsonBody);
        ...
        for (let i = 0; i < keyValuePairs.length; i++) {
            ...
            for (let j = 0; j < bodyMappingParametersList.length; j++) {
                const element = bodyMappingParametersList[j];
                if (key === element) {
                    logger.debug("key found:", key);
                    break;
                }
            }
            logger.debug("restrict key:", key, element['value']);
            valuetoRestrict = element['value'];
            break;
        }
        restictBodyFlag = isValuePresentInAccountIds(valuetoRestrict);
    } catch (error) {
        logger.error("Error while parsing incoming http request json body for account restriction:", error);
    }

    return restictBodyFlag;
}
    ...
    try {
        ...
        switch (contentType) {
            case APPLICATION_JSON:
                return checkForAccountRestrictionToJsonBody(request);
                break;
        }
    } catch (error) {
...
function isHttpRequestRestictToAccount(request) {
    logger.debug('requet is:', request);
    ...
    console.log("canRestictBasedOnBody:", canRestictBasedOnBody);
    return true;
}
