...
function ServiceMapConnections(outBoundConnectionsList, inBoundConnectionsList) {
    ...
    externalConnectionEvent.jsonName = 'sec-application-connections';
    externalConnectionEvent.eventType = 'sec-application-connections';
    ...
    externalConnectionEvent.entityIdApiGroup = generateEntityGroup();
...
function generateEntityGroup() {
    let entityIdApiGroup = EMPTY_STR;
    try {
        let apiEndpoints = require('../../../instrumentation-security/core/route-manager').getAllAPIEndPoints();
        entityIdApiGroup = require('./sha-size-util').getSHA256ForData(JSON.stringify(apiEndpoints));
    } catch (error) {

    }

    return entityIdApiGroup;
}
