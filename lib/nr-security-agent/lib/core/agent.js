...
Agent.prototype.setApplicationInfo = function setApplicationInfo(applicationInfoObject) {
    ...
    if (NRAgent && NRAgent.config && NRAgent.config.entity_guid) {
        ...
        initLogger.info(LOG_MESSAGES.INIT_APPINFO_SEND, JSON.stringify(applicationInfoObject));
...
Agent.setNRAgent = function setNRAgent(agent) {
    if (agent) {
        NRAgent = agent;
        process.env.account_id = NRAgent.config.account_id;
        process.env.agent_run_id = NRAgent.config.run_id; 
    }
