...
function initialize(applicationUUID) {
    try {
        let logLevel = NRAgent && NRAgent.config && NRAgent.config.logging.enabled ? NRAgent.config.logging.level : 'off';
        ...
        if (logLevel && !validLogLevels.includes(logLevel.toLowerCase())) {
            console.error("Security agent logging failed. Please check log level.")
            logLevel = 'off';
            isLoggerOk = false;
        }
        const appenders = {};
        ...
        console.error(`Security agent is unable to log into file: ${logFileName}.`, error.message);
        isLoggerOk = false;
    }
}
