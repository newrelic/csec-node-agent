...
const LinkingMetaData = require('./lib/core/LinkingMetadata');
...
// const SKIP_APPLICATION_REGEX = /(npm|ng|pm2|node-pre-gyp|fsevents\/install|node-gyp.js|\.bin\/prebuild-install|lint-staged|husky|update-notifier\/check.js|bin\/postinstall)$/g
...
function initialize() {
    ...
    if ((extractApplicationDef(getProcCMDLineSanitized(applicationInfoModule), applications) || extractApplicationDef(process.argv, applications)) &&
        ...
        initLogger.info(LOG_MESSAGES.COLLETOR_INFO, applicationInfo.collectorVersion, applicationInfo.jsonVersion, applicationInfo.buildNumber);
        ...
        logger.info('Application Metadata:', JSON.stringify(process.pjson));
