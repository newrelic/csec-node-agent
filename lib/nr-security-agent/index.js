    ...
    if (NRAgent) {
        NRAgent.on('started', () => {
            ...
            if (NRAgent.config.security.enabled || !NRAgent.config.high_security) {
                ...
                NRLogger.warn("Failed to Start New Relic Node.js Security Agent. Please check security.enabled and high_security flags");
            }
        });
    }
...
/**
 * Utility to resolve validator host based on APM host
 */
function resolveValidatorHost() {
    if (NRAgent.config.host === 'collector.newrelic.com') {
        NRAgent.config.security.validator_service_url = 'wss://csec.nr-data.net';
    }
    else if (NRAgent.config.host === 'collector.eu.newrelic.com') {
        NRAgent.config.security.validator_service_url = 'wss://csec.eu01.nr-data.net ';
    }
    else if (NRAgent.config.host === 'gov-collector.newrelic.com') {
        NRAgent.config.security.validator_service_url = 'wss://csec-gov.nr-data.net';
    }
    else if (NRAgent.config.host === 'staging-collector.newrelic.com') {
        NRAgent.config.security.validator_service_url = 'wss://csec-staging.nr-data.net';
    }
