...
function initialize() {
    ...
    initLogger.info("[STEP-3] => Gathering information about the application");
    if (NRAgent) {
        NRAgent.on('started', () => {
    ...
    if ((extractApplicationDef(getProcCMDLineSanitized(applicationInfoModule), applications) || extractApplicationDef(process.argv, applications)) &&
        ...
        logger.info(LOG_MESSAGES.AGENT_INFO, applicationInfo.collectorVersion, applicationInfo.jsonVersion);
